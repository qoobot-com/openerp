...
- API 调用示例：前端 API 文件位于 `src/api/permission.ts`
...
**前端 API 封装示例**：
```typescript
// qooerp-frontend/packages/web/src/api/permission.ts
import request from '@/utils/request'

export const permissionApi = {
  // 角色管理
  roleList: (params: RoleQuery) => {
    return request.get<PageResult<Role>>('/api/permission/role/list', { params })
  },
  roleCreate: (data: RoleCreate) => {
    return request.post<void>('/api/permission/role/create', data)
  },
  roleUpdate: (id: number, data: RoleUpdate) => {
    return request.put<void>(`/api/permission/role/${id}`, data)
  },
  roleDelete: (id: number) => {
    return request.delete<void>(`/api/permission/role/${id}`)
  },
  roleAssignPermission: (roleId: number, permissionIds: number[]) => {
    return request.post<void>(`/api/permission/role/${roleId}/permissions`, { permissionIds })
  },

  // 菜单管理
  menuTree: () => {
    return request.get<MenuNode[]>('/api/permission/menu/tree')
  },
  menuCreate: (data: MenuCreate) => {
    return request.post<void>('/api/permission/menu/create', data)
  },
  menuUpdate: (id: number, data: MenuUpdate) => {
    return request.put<void>(`/api/permission/menu/${id}`, data)
  },
  menuDelete: (id: number) => {
    return request.delete<void>(`/api/permission/menu/${id}`)
  },

  // 用户权限
  userPermissions: (userId: number) => {
    return request.get<string[]>(`/api/permission/user/${userId}/permissions`)
  },
  userRoles: (userId: number) => {
    return request.get<Role[]>(`/api/permission/user/${userId}/roles`)
  },
  assignUserRoles: (userId: number, roleIds: number[]) => {
    return request.post<void>(`/api/permission/user/${userId}/roles`, { roleIds })
  },

  // 数据权限
  dataScope: () => {
    return request.get<DataScope[]>('/api/permission/data-scope')
  },
  updateDataScope: (userId: number, dataScope: DataScope) => {
    return request.put<void>(`/api/permission/user/${userId}/data-scope`, dataScope)
  },

  // 审计日志
  auditLog: (params: AuditLogQuery) => {
    return request.get<PageResult<AuditLog>>('/api/permission/audit/log', { params })
  }
}
```
...
| 版本 | 日期 | 变更内容 | 作者 |
|------|------|----------|------|
| 1.0.0 | 2026-02-18 | 初始版本，添加前端技术栈 | Auto |
