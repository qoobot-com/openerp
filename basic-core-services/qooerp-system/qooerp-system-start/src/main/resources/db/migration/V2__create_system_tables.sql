...
COMMENT ON COLUMN system_log.location IS '请求地点';
COMMENT ON COLUMN system_log.browser IS '浏览器';
COMMENT ON COLUMN system_log.os IS '操作系统';
...
COMMENT ON COLUMN system_announcement.announcement_type IS '公告类型：urgent-紧急，important-重要，normal-普通';
COMMENT ON COLUMN system_announcement.target_type IS '目标类型：all-全部用户，department-指定部门，role-指定角色';
COMMENT ON COLUMN system_announcement.target_ids IS '目标ID列表（JSON格式）';
